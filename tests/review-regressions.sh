...
minimax_image_source=$(nix build --no-link --print-out-paths \
  '.#nixosConfigurations.Luna-Server.config.system.build.hermesMinimaxImagePlugin')
minimax_video_source=$(nix build --no-link --print-out-paths \
  '.#nixosConfigurations.Luna-Server.config.system.build.hermesMinimaxVideoPlugin')
hermes_extensions_installer=$(nix build --no-link --print-out-paths \
  '.#nixosConfigurations.Luna-Server.config.system.build.hermesServerExtensionsInstaller')
hermes_python="$(nix eval --raw \
  '.#nixosConfigurations.Luna-Server.pkgs.hermes-agent.hermesVenv.outPath')/bin/python3"
...
server_bolt=$(nix eval --json '.#nixosConfigurations.Luna-Server.config.services.hardware.bolt.enable')
assert_eq true "$server_bolt" "Luna-Server persistent Mate SE authorization service"
