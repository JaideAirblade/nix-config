{
  ...
  inputs = {
    ...
    # Honcho v3 was removed 2026-08-14 — no longer in use on any host.
  ...
  outputs = inputs@{ self, flake-parts, ... }:
    let
      ...
      dendriticExceptions = {
        ...
        "hosts/Luna-Server/hardware-configuration.nix" = "generated by nixos-generate-config; direct host import";
        ...
        "hosts/Luna-Server/boot-order.nix" = "dendritic lower-level NixOS module; imported directly by hosts/Luna-Server/default.nix";
        ...
        # pattern as `hosts/Luna-Server/boot-order.nix`. The wiring
      ...
      perSystem = { pkgs, system, ... }:
        ...
        in
        {
          ...
          checks = {
            ...
            server-greeter =
              assert !self.nixosConfigurations.Luna-Server.config.programs.steam.gamescopeSession.enable;
              pkgs.runCommand "luna-server-greeter-check" { } ''
                touch $out
              '';
