...
# ## Selecting the role
#
# Select `config.nixos.modules.remoteAccess` in the host entry point, then
# contribute the required settings from a host feature:
#   {
...
_:
{
  nixos.modules.remoteAccess =
    ...
    let
      ...
      sopsKey =
        if cfg.privateKeySecret != "" then cfg.privateKeySecret
    ...
    in
    {
      ...
      config = {
        assertions = [
          {
            assertion = sopsKey != "awg_private_key";
            message = "amneziawg-mesh requires a distinct per-host private key secret";
          }
          {
            assertion = lib.any (host: host.endpoint != "") (lib.attrValues otherHosts);
            message = "amneziawg-mesh needs at least one peer with a bootstrap endpoint";
          }
        ];
