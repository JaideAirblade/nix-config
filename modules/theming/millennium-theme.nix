...
#
# Material-Theme's stock matugen.js reloads its stylesheet every 1.5s in every
# Steam web context. The sync script replaces it with a low-churn watcher that
# only replaces the injected CSS when its contents actually change.
_:
{
  nixos.modules.common =
    ...
    let
      ...
      syncScript = pkgs.writeShellScriptBin "sync-millennium-theme" ''
        ...
        matugen_css="$THEME_DIR/css/main/colors/matugen.css"
        matugen_tmp="$matugen_css.tmp"
        if [[ -f "$COLORS_FILE" ]]; then
          generate_matugen_css "$COLORS_FILE" > "$matugen_tmp"
        else
          # Fallback: use the matugen.css that shipped with Material-Theme
          # (it has reasonable default colors)
          cp -f "$STATIC_THEME/css/main/colors/matugen.css" "$matugen_tmp"
        fi

        mv -f "$matugen_tmp" "$matugen_css"

        # The live watcher requests the copy exposed by steamloopback.host.
        # Publish it only after generation, and atomically: the old ordering
        # copied the previous palette before generating the new one.
        mkdir -p "$SKINS_DIR/css/main/colors"
        cp -f "$matugen_css" "$SKINS_DIR/css/main/colors/matugen.css.tmp"
        mv -f "$SKINS_DIR/css/main/colors/matugen.css.tmp" "$SKINS_DIR/css/main/colors/matugen.css"

        # Replace Material-Theme's 1.5-second unconditional stylesheet reload.
        # This keeps live DMS updates while avoiding constant stylesheet reparses
        # in every Steam web context. CSS is changed only when its content changes.
        install_live_color_watcher() {
          local watcher="$THEME_DIR/js/main/colors/matugen.js"
          mkdir -p "$(dirname "$watcher")"
          cat > "$watcher.tmp" <<'JS'
        (() => {
          const url = "https://millennium.host/v1/themes/Material-Theme/css/main/colors/matugen.css";
          const style = document.createElement("style");
          style.dataset.dmsMatugen = "true";
          ...
          async function refresh() {
            try {
              ...
              // Steam can destroy a web context while a request is in flight.
            }
          }
        ...
        JS
          mv -f "$watcher.tmp" "$watcher"
          mkdir -p "$SKINS_DIR/js/main/colors"
          cp -f "$watcher" "$SKINS_DIR/js/main/colors/matugen.js.tmp"
          mv -f "$SKINS_DIR/js/main/colors/matugen.js.tmp" "$SKINS_DIR/js/main/colors/matugen.js"
        }
        install_live_color_watcher

        # Steam renders each Supernav dropdown in a separate opaque XWayland
        # CEF surface. A rounded outer panel therefore exposes black quarter-
        # circle wedges rather than the Steam window below. Millennium injects
        # this dedicated stylesheet after Material-Theme's own files.
        install_dms_fixes() {
          local fixes_css="$THEME_DIR/css/main/dms-fixes.css"
          mkdir -p "$THEME_DIR/css/main" "$SKINS_DIR/css/main"
          cat > "$fixes_css.tmp" <<'CSS'
        /* DMS XWayland Supernav corner fix */
        html.Library_Supernav .PP7LM0Ow1K5qkR8WElLpt._2yAm5LY_eu-Vg_52l0HFlM,
        html.Store_Supernav .PP7LM0Ow1K5qkR8WElLpt._2yAm5LY_eu-Vg_52l0HFlM,
        html.Community_Supernav .PP7LM0Ow1K5qkR8WElLpt._2yAm5LY_eu-Vg_52l0HFlM,
        html.Profile_Supernav .PP7LM0Ow1K5qkR8WElLpt._2yAm5LY_eu-Vg_52l0HFlM {
            border-radius: 0 !important;
            overflow: hidden !important;
            background-clip: padding-box !important;
        }
        CSS
          mv -f "$fixes_css.tmp" "$fixes_css"
          cp -f "$fixes_css" "$SKINS_DIR/css/main/dms-fixes.css.tmp"
          mv -f "$SKINS_DIR/css/main/dms-fixes.css.tmp" "$SKINS_DIR/css/main/dms-fixes.css"

          # Undo the earlier in-place prototype if this existing deployment has it.
          cp -f "$STATIC_THEME/css/main/restyle/md3menu.css" "$THEME_DIR/css/main/restyle/md3menu.css"
          mkdir -p "$SKINS_DIR/css/main/restyle"
          cp -f "$THEME_DIR/css/main/restyle/md3menu.css" "$SKINS_DIR/css/main/restyle/md3menu.css"
        }
        install_dms_fixes
        ...
        if command -v python3 >/dev/null 2>&1; then
          python3 - "$MILLENNIUM_CONFIG" "$matugen_css" "$THEME_DIR/skin.json" "$SKINS_DIR/skin.json" <<'PYCFG'
        import json, os, re, sys
        path = sys.argv[1]
        colors_path = sys.argv[2]
        skin_paths = sys.argv[3:]
        try:
            with open(path) as f:
                data = json.load(f)
        except Exception:
        ...
        material["DMS Fixes"] = "On"

        # Add a dedicated, directly injected stylesheet. This avoids imported
        # CSS caching and guarantees the Supernav fix loads after theme styles.
        dms_condition = {
            "description": "NixOS/DMS integration fixes.",
            "default": "On",
            "tab": "Style",
            "values": {
                "Off": {},
        ...
        for skin_path in skin_paths:
            try:
                ...
                continue
            skin.setdefault("Conditions", {})["DMS Fixes"] = dms_condition
            skin_tmp = skin_path + ".tmp"
            with open(skin_tmp, "w") as f:
                json.dump(skin, f, indent=4)
                f.write("\n")
            os.replace(skin_tmp, skin_path)

        # Millennium persists RootColors as higher-priority values in config.json.
        # Keep those values synchronized too, otherwise they override the current
        # DMS palette even when matugen.css itself is correct.
        try:
            css = open(colors_path).read()
        except OSError:
            css = ""
        palette = {
            name: value.strip()
            for name, value in re.findall(
                r"(--md-sys-color-[\w-]+)\s*:\s*([^;]+);", css
            )
        }
        theme_colors = data["themes"].setdefault("themeColors", {})
        persisted = theme_colors.setdefault("Material-Theme", {})
        persisted.update(palette)

        # Follow DMS light/dark mode from the generated background itself.
        background = palette.get("--md-sys-color-background", "")
        match = re.fullmatch(r"rgb\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\)", background)
        if match:
            rgb = [int(channel) / 255 for channel in match.groups()]
            linear = [c / 12.92 if c <= 0.04045 else ((c + 0.055) / 1.055) ** 2.4 for c in rgb]
            luminance = 0.2126 * linear[0] + 0.7152 * linear[1] + 0.0722 * linear[2]
            material["Appearance"] = "Light" if luminance > 0.5 else "Dark"
