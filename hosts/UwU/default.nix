...
{ config, inputs, ... }:
{
  flake.nixosConfigurations.UwU = inputs.nixpkgs.lib.nixosSystem {
    ...
    modules = [
      ...
      # Noctalia desktop shell — opt-in for UwU only (Stage 1). The role
      # module lives at modules/wm/noctalia/noctalia.nix; the per-host
      # wiring that disables DMS/mango and enables niri lives at
      # hosts/UwU/desktop/noctalia-host.nix (auto-discovered). TSBW-W01800
      # and other hosts still consume DMS via nixos.modules.common.
      config.nixos.modules.noctalia
