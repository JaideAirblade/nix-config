# Mango → systemd session bridge for UwU.
#
# Root-cause note (2026-08-19): dms setup rewrote
...
# when the DMS daemon+UI die (silent pair-kill, no coredump/OOM — same
# crash streak observed on UwU-Server 2026-08-13..15), nothing respawns
# them and the panel stays dead until re-login. It also leaves
# graphical-session.target inactive, which breaks xdg-desktop-portal
# (Dependency failed for Portal service).
#
# This target — together with the two exec-once lines restored in
# mango's config.conf (exec-once=dbus-update-activation-environment
# --systemd --all + exec-once=systemctl --user start mango-session.target)
# — puts DMS back under dms.service (Restart=on-failure), so a crash
# becomes a ~2s blink and journald finally records the exit signal.
# Mirrors the proven setup in hosts/UwU-Server/desktop/mango-session.nix
# and hosts/TSBW-W01800/desktop/{mango,dms}.nix.
{
  nixos.hosts."UwU" =
    { lib, ... }: {
      ...
      programs.dank-material-shell = {
        systemd = {
          ...
          # session"), so a mid-switch restart just stops+fails. Worse, stopping
          ...
          # (Same rationale as hosts/UwU-Server/desktop/mango-session.nix and
          # hosts/TSBW-W01800/desktop/dms.nix.)
