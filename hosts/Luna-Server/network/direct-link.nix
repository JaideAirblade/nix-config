# Direct-link networking: Luna-Server acts as gateway + DNS for UwU desktop.
#
# Physical topology (changed 2026-08-08):
#   Luna-Server eth0 (r8152 USB 2.5GbE) -- UwU enp10s0 (Realtek 1GbE)
#
# UwU no longer goes through the apartment router for internet -- it routes
# through Luna-Server instead. Luna-Server still gets its own internet via
...
# (recursive resolver) -- both on Luna-Server, listening on the direct-link
# interface so UwU (and only UwU) can reach them.
#
# Subnet: 10.10.0.0/30
#   Luna-Server eth0  -> 10.10.0.1/30  (gateway + DNS)
...
_:
{
  nixos.hosts."Luna-Server" =
    ...
    let
      # Direct-link interface on Luna-Server (r8152 USB 2.5GbE adapter).
      linkIface = "eth0";
      linkIP = "10.10.0.1";
    ...
    in
    {
      ...
      # --- IP forwarding + NAT (Luna-Server as gateway for UwU) --------------
      boot.kernel.sysctl = {
      ...
      services.adguardhome = {
        ...
        settings = {
          ...
          dns = {
            ...
            # hosts/Luna-Server/default.nix). Port 53 is taken by
