# Luna-Server (Beelink SER10 Max) configuration entry point.
...
# `collectModules` walker. That file defines `nixos.hosts."Luna-Server"` as a
...
# `config.nixos.hosts."Luna-Server"` read on line 20 is what wires
# `disk-layout.nix` into the system build.
{ config, inputs, ... }:
{
  flake.nixosConfigurations.Luna-Server = inputs.nixpkgs.lib.nixosSystem {
    system = "x86_64-linux";
    ...
    modules = [
      ...
      config.nixos.hosts."Luna-Server"
      ...
      { nixpkgs.overlays = [ (import ../../overlays/millennium.nix { millennium-input = inputs.millennium; }) ]; }
      {
        ...
        services.netbirdMesh = {
          ...
          # to that loopback address (see hosts/Luna-Server/network/
          ...
          # against `which netbird`. Even though Luna-Server is
