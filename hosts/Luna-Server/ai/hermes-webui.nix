...
# "luna" user on Luna-Server benefits, so this module is registered against
# `nixos.hosts."Luna-Server"` and not `nixos.modules.common`.
#
  ...
  nixos.hosts."Luna-Server" =
    { config, lib, pkgs, ... }:
    {
      ...
      sops.secrets.hermes_webui_password = {
        sopsFile = "${inputs.nixos-secrets}/secrets/Luna-Server/hermes-webui.yaml";
