# Chrony NTP + NTS server on Luna-Server.
#
...
#
# Stratum: 2. Luna-Server syncs upstream to the public pool; clients on the
# mesh sync to Luna-Server. (Stratum-1 would need a hardware ref like a
...
# `nixos.hosts."Luna-Server" = { pkgs, lib, ... }:` is the NixOS module
# that DOES get `pkgs` from the standard module system. Same pattern as
# hosts/Luna-Server/network/direct-link.nix and hosts/Luna-Server/boot-order.nix.
_:

let
  # Address Luna-Server advertises itself as on each interface the clients
  # reach it from. These come from:
  #   * netbirdMesh (modules/network/netbird-mesh.nix)         -> 100.77.228.137
  #   * direct-link (hosts/Luna-Server/network/direct-link.nix) -> 10.10.0.1
  ...
  # hosts/Luna-Server/server/dashboard.nix for the lego setup.)
