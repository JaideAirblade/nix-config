...
#
# Runs on UwU-Server behind nginx at search.jaidechan.moe.
# Exposed only on the Tailscale IP (100.102.183.94) like all other services
    ...
    in
    {
      ...
      services.searx = {
        ...
        settings = {
          ...
          server = {
            # Bound to loopback — nginx fronts it via the Tailscale IP.
            ...
            # your own Tailscale IP after a few quick requests during
            # debugging. With only ~1 user (you) hitting it, the limiter
            # adds nothing. Disable for now; re-enable if you ever expose
            # this externally.
            limiter = false;
            # Not a public instance. Anyone reaching it is on Tailscale.
      ...
      # (which listens on the Tailscale IP only, per dashboard.nix) can
